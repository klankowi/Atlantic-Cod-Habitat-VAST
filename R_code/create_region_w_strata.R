...
head(shpsplit@data)
plot(shpsplit, col=as.factor(shpsplit$STRATA))
...
plot(shp.all, col=as.factor(shp.all$STRATA))

# Combine
shp <- rbind(shpsplit, shp.all)
plot(shp, col=as.factor(shp$STRATA))
plot(shp[shp$STRATA =='ALL',], col=as.factor(shp$STRATA[shp$STRATA == "ALL"]))
table(shp$STRATA)
...
ggplot(ecodata::coast) +
  geom_sf(data=region_polygon_sf, 
          aes(fill=STRATA),
          alpha=0.5) +
  geom_sf() +
...
table(region_sf$STRATA)

# Plot
ggplot(ecodata::coast) +
  geom_sf(data=region_sf, aes(col=STRATA)) +
  geom_sf() +
  facet_wrap(vars(STRATA)) +
  coord_sf(xlim=c(-77, -65), ylim=c(35, 48))

# View results
region$STRATA <- as.factor(region$STRATA)
str(region)
...
saveRDS(region, file = here("Data/VAST_input/VAST_regions/user_region_wstrata.rds"))
