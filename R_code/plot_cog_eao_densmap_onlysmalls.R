...
# Load data
load(here('VAST_runs/refine_effort_onlysmalls/only_smalls.RData'))
...
names(size.list.cog) <- c('Small')  
rm(SD_mean_Z_ctm, SD_mean_Z_ctm_small)
...
names(size.list.eao) <- c('Small')       
rm(SD_effective_area_ctl,  SD_effective_area_ctl_small)
...
for(i in 1:length(size.list.cog)){ # Number of sizes
  #for(j in 1:dim(season.list.cog[[i]])[1]){ # Number of categories
    ...
    both <- annotate_figure(both, top=text_grob(paste0('Small',
                                                       ' size class'),
                                                color='black',
                                                face='bold',
                                                size=16,
...
for(i in 1:length(size.list.cog)){ # Categories
    ...
    spring <- ggplot() +
      ...
      coord_sf(xlim=c(340000, 440000),
               ylim=c(4570000, 4705000)) +
