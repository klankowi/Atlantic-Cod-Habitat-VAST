...
for(i in seq_along(index_areas)){
  index_area_temp<- st_read(paste0(here::here("", "Data/GIS"), '/', index_areas[i], "_UTM.shp"))
...
vast_extrap_grid<- vast_make_extrap_grid(region_shapefile = region_shape, index_shapes = index_area_shapes, 
                                         #strata.limits = strata_use, 
                                         cell_size = 1000)
#user_region <- readRDS(here('Data/VAST_input/VAST_regions/user_region_all.rds'))
...
rm(list=setdiff(ls(), c("scaled.covars", "settings", 'strata_use', 'survs',
                        'vast_extrap_grid', 'cat.labs', 'year.labs')))
gc()

#### Run model ####
working_dir <- here::here("VAST_runs/add_climate_aja")
...
cat.labs <- c('Unknown', 'Small', 'Medium', 'Large')
...
save.image('add_climate_aja.RData')
