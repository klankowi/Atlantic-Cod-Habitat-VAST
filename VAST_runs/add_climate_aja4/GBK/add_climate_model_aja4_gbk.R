...
index_areas<- c("GBK", "EGOM", "SNE", "WGOM")
...
strata_use<- data.frame(STRATA = c( "GBK", "EGOM", "SNE", "WGOM", "ALL"))
...
working_dir <- here::here("VAST_runs/add_climate_aja4/GBK")
...
extrap_info_aja_gbk <- make_extrapolation_info(Region="User",
                                            strata.limits=strata_use,
                                            input_grid = vast_extrap_grid,
                                            max_cells = 2000
)
head(extrap_info_aja_gbk$a_el)
head(extrap_info_aja_gbk$Data_Extrap)
write_rds(extrap_info_aja_gbk,
          here('Data/VAST_input/extrap_info_aja_gbk.RDS'))
#extrap_info_aja_wgom <- readRDS(here('Data/VAST_input/extrap_info_aja_wgom.RDS'))

# Remove intermediates
rm(list=setdiff(ls(), c("scaled.covars", "settings", 'strata_use', 'survs',
                        'vast_extrap_grid', 'extrap_info',
                        'working_dir', 'extrap_info_aja_gbk')))
gc()
...
fit = fit_model( 
  ...
  # Call spatial 
    input_grid=vast_extrap_grid,
    "extrapolation_list" = extrap_info_aja_gbk,
...
save.image('add_climate_aja4_gbk.RData')
