...
index_areas<- c("SNE", "EGOM", "GBK", "WGOM")
...
strata_use<- data.frame(STRATA = c( "SNE", "EGOM", "GBK", "WGOM", "ALL"))
...
working_dir <- here::here("VAST_runs/add_climate_aja4/SNE")
...
extrap_info_aja_sne <- make_extrapolation_info(Region="User",
                                            strata.limits=strata_use,
                                            input_grid = vast_extrap_grid,
                                            max_cells = 2000
)
head(extrap_info_aja_sne$a_el)
head(extrap_info_aja_sne$Data_Extrap)
write_rds(extrap_info_aja_sne,
          here('Data/VAST_input/extrap_info_aja_sne.RDS'))
#extrap_info_aja_wgom <- readRDS(here('Data/VAST_input/extrap_info_aja_wgom.RDS'))

# Remove intermediates
rm(list=setdiff(ls(), c("scaled.covars", "settings", 'strata_use', 'survs',
                        'vast_extrap_grid', 'extrap_info',
                        'working_dir', 'extrap_info_aja_sne')))
gc()
...
fit = fit_model( 
  ...
  # Call spatial 
    input_grid=vast_extrap_grid,
    "extrapolation_list" = extrap_info_aja_sne,
...
save.image('add_climate_aja4_sne.RData')
